...
Templates to monitor PostgreSQL by Zabbix adapted by Denis Matveev @ Ignitia AB.
This template was tested on PostgreSQL versions 13 on Linux Ubuntu.
...
2. Copy `postgresql/` to Zabbix agent home directory `/etc//zabbix/scripts/`. The `postgresql/` directory contains the files needed to obtain metrics from PostgreSQL.

3. Copy `postgresql.conf` to Zabbix agent configuration directory `/etc/zabbix/zabbix_agentd.d/` and restart Zabbix agent service.
...
5. Import `postgresql_template.yaml` to Zabbix and link it to the target host and setup PG.PASSWORD macro in the template(not in inherited section)
